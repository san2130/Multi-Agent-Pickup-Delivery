...
def mark(m1,m2,m3):
    global st,gc,tc,re,pmsg,bots,dropping,pkg1,pkg2,destbot,path_pub1,path_pub2,servo_pub1,servo_pub2,ind1,ind2,acp,ind11,ind22,indx1,indy1,path_pub3,servo_pub3
    pathinitialise()
    re=[0,0,0]
    dropping=[1,1,1]
    s=0
    ...
    msgl=[m1,m2,m3]
    st=[0,0,0]
    path_pub=[path_pub1,path_pub2,path_pub3]
    servo_pub=[servo_pub1,servo_pub2,servo_pub3]
    nopub=[0,0,0]
    lm=[len(m1.poses),len(m2.poses),len(m3.poses)]
    for i in range(bots):
        offx=0
        offy=0
        ...
        if st[j]==0:
          ...
          gc[j]=(mapped[0],mapped[1])
           ...
           if (gc[j][0]==3 and gc[j][1]==1):
              ...
              pmsg[j].poses.append(indy1)
              pmsg[j].poses.append(msgl[j].poses[i-1])
           elif (gc[j][0],gc[j][1]) in actualpre:
               nopub[j]=1
...
def induct(x,y):
   if (x==3 and y==1) or (x==5 and y==1):
     ...
     return False
def targetpt():
    global gc,tc,st,ptg,pkg1,pkg2,bots,f,tp1,tp2,actualpre
    ipl=[]
    spl=[]
    # print("f=",f)
    if f==0:
    ...
    preinductpts=[(1,1),(7,1)]
    mini=500
    for i in ipl:
      for p in preinductpts:
            disi=dis(i[0][0],i[0][1],p[0],p[1])
            if disi<mini:
               posj=i
               mini=disi
